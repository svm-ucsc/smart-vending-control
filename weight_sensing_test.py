...
LANE_ROTATIONS = 6.5                # Number of rotations needed to dispense one item (will change)
...
class Machine():
  ...
  def ItemsReceived(self) -> bool:
    ...
    self.sensor.set_prev_read(self.sensor.get_grams())
    while (abs(self.sensor.detect_change(0.1)) < weight_on_plat):
